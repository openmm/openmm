...
def run_simulation():
    ...
    dt = 0.001  # ps (1 fs)
    equilibration_time_ps = 50.0  # 50 ps equilibration (shorter for testing)
    production_time_ps = 200.0    # 200 ps production
    ...
    print(f"  Equilibration: {equilibration_time_ps} ps")
    print(f"  Production: {production_time_ps} ps")
    ...
    try:
        ...
        print(f"  CavityForce added with λ = {lambda_coupling}")
        print(f"  Coupling is ON from the start (no equilibration phase)")
    ...
    save_interval = 1  # Save dipole every step (1 fs)
    ...
    for step in range(total_steps):
        ...
        if (step + 1) % report_interval == 0:
            ...
            print(f"  [{100*(step+1)/total_steps:5.1f}%] t={sim_time:6.1f} ps | "
                  f"PE={pe:7.1f} kJ/mol | Cavity: ({cavity_pos[0]:.3f}, {cavity_pos[1]:.3f}, {cavity_pos[2]:.3f}) nm | "
