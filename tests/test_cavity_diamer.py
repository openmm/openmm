...
def create_diamer_system(num_molecules=50, fraction_OO=0.8, box_size_nm=2.0, 
    ...
    # cav-hoomd uses: V = k * (r - r0)²
    # OpenMM uses: V = 0.5 * k * (r - r0)²
    # Therefore we need to DOUBLE the force constants!
    # cav-hoomd values: k_OO = 2*0.36602 = 0.73204, k_NN = 2*0.71625 = 1.4325
    k_OO_au = 2 * 0.73204  # Hartree/Bohr² (doubled for OpenMM)
...
def run_test():
    ...
    num_molecules = 50
    box_size_nm = 2.5
    temperature_K = 100.0  # Use 100 K like cav-hoomd for clear vibrational peaks!
    
    # Cavity parameters
    # Pass omega_c in ATOMIC UNITS (Hartree) to match cav-hoomd convention
    # omega_c will be converted internally to proper OpenMM units
    omegac_au = 0.00913  # Hartree (energy = ℏω in atomic units where ℏ=1)
    
    lambda_coupling = 0.001  # Dimensionless coupling
    photon_mass = 1.0  # amu
    
    # Simulation parameters
    dt = 0.001  # ps (1 fs)
    equilibration_time_ps = 100.0  # 100 ps equilibration
    production_time_ps = 900.0  # 900 ps production (total 1 ns)
    ...
    print(f"  Total time: {equilibration_time_ps + production_time_ps} ps (1 ns)")
    print(f"  Dipole output: Every timestep (1 fs) for accurate IR spectrum")
    ...
    print("\n--- Adding Cavity Force ---")
    try:
        # Create CavityForce - pass omega_c in atomic units
        cavity_force = openmm.CavityForce(cavity_index, omegac_au, 0.0, photon_mass)  # Start with lambda=0
        cavity_force.setCouplingOnStep(equilibration_steps, lambda_coupling)  # Turn on at equilibration end
        system.addForce(cavity_force)
        print(f"  CavityForce added successfully")
        print(f"  Omega_c: {omegac_au:.6f} a.u. (Hartree)")
        print(f"  Photon mass: {photon_mass:.1f} amu = {photon_mass * 1822.888:.1f} a.u.")
        ...
        print(f"  Initial lambda: 0.0")
        print(f"  Coupling will turn on at step {equilibration_steps} with lambda={lambda_coupling}")
    ...
    try:
        ...
        print(f"  Using CUDA platform for GPU acceleration")
    except Exception:
        platform = openmm.Platform.getPlatformByName('Reference')
        print(f"  Using Reference platform (CPU-only, will be slow)")
    ...
    for i in range(num_reports):
        ...
        for step in range(report_interval):
            integrator.step(1)
            step_counter += 1
            if (step_counter % dipole_output_interval_steps) == 0:
    ...
    output_file = "cavity_diamer_dipole.npz"
