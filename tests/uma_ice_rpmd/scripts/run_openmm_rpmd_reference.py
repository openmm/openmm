...
# Repo layout: this file lives under ``tests/uma_ice_rpmd/scripts/``.
...
def _lammps_data_to_xyz(data_path: Path, molecules: int) -> Path:
    ...
    xyz_path = _ROOT / "pipeline_out" / f"init_openmm_rpmd_{molecules}.xyz"
...
def main() -> None:
    ...
    ap.add_argument("--order-csv", type=Path, default=_ROOT / "pipeline_out" / "ice_order_openmm_rpmd.csv")
    ap.add_argument(
        "--order-every",
        type=int,
        default=10,
    ...
    cmd = [
        sys.executable,
        str(_ROOT / "test_uma_ice_rpmd.py"),
        "--input", str(xyz_path),
    ...
    subprocess.check_call(cmd, cwd=str(_ROOT), env=os.environ.copy())
