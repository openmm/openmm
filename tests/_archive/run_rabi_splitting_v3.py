...
Version 3: With finite-q displacement correction and proper equilibration.
...
def run_simulation():
    ...
    print("(v3 - with finite-q displacement correction)")
    print("=" * 70)
    ...
    # Add CavityParticleDisplacer for finite-q displacement correction
    print("\n--- Adding Cavity Particle Displacer (finite-q correction) ---")
    try:
        ...
        print(f"  Will compute q_eq = -(λ/ω_c) * d_xy at step {equilibration_steps}")
    except AttributeError as e:
        print(f"  WARNING: CavityParticleDisplacer not available: {e}")
        print("  Proceeding without finite-q correction (may cause instability)")
    ...
    for step in range(total_steps):
        ...
        if (step + 1) % report_interval == 0:
            ...
            print(f"  [{100*(step+1)/total_steps:5.1f}%] t={sim_time:6.1f} ps | {phase} | "
                  f"PE={pe:7.1f} kJ/mol | Cavity: ({cavity_pos[0]:.4f}, {cavity_pos[1]:.4f}, {cavity_pos[2]:.4f}) nm | "
    ...
    np.savez(output_file,
             ...
             cavity_positions_nm=cavity_positions,
             metadata=metadata)
    ...
    print("\n--- Quick Analysis ---")
    cavity_positions = np.array(cavity_positions)
    print(f"  Cavity x range: [{cavity_positions[:,0].min():.4f}, {cavity_positions[:,0].max():.4f}] nm")
    print(f"  Cavity y range: [{cavity_positions[:,1].min():.4f}, {cavity_positions[:,1].max():.4f}] nm")
    print(f"  Cavity z range: [{cavity_positions[:,2].min():.4f}, {cavity_positions[:,2].max():.4f}] nm")
