...
static void          myGetOpenMMState(MyOpenMMData*, bool wantEnergy,
                                      double& time, double& energy,  double& pertenergy,
                                      MyAtomInfo atoms[]);
...
static void
myWritePDBFrame(int frameNum, double timeInPs, double energyInKcal, double pertenergyInKcal, 
                const MyAtomInfo atoms[]) 
{
    ...
    printf("REMARK 250 time=%.3f ps; pertenergy=%.3f kcal/mole\n", 
	   timeInPs, pertenergyInKcal);
...
int main() {
    ...
    try {
      double        time, energy, pertenergy;
        ...
        myGetOpenMMState(omm, WantEnergy, time, energy, pertenergy, atoms);
        myWritePDBFrame(1, time, energy, pertenergy, atoms);
...
struct MyOpenMMData {
  MyOpenMMData() : system(0), context(0), integrator(0), atm(0) {}
  ~MyOpenMMData() {delete system; delete context; delete integrator; delete atm;}
...
myInitializeOpenMM( const MyAtomInfo    atoms[],
                    ...
                    std::string&        platformName) 
{
    std::cout << "Loading plugins from " << OpenMM::Platform::getDefaultPluginsDirectory() << std::endl;
    // Load all available OpenMM plugins from their default location.
    OpenMM::Platform::loadPluginsFromDirectory
       (OpenMM::Platform::getDefaultPluginsDirectory());
    std::cout << "Number of platforms:" << OpenMM::Platform::getNumPlatforms() << std::endl;
    ...
    std::vector<int> atmvariableforcegroups(1, 1);
    double lmbd = 0.5;
    double lambda1 = lmbd;
    double lambda2 = lmbd;
    double alpha = 0.0;
...
myGetOpenMMState(MyOpenMMData* omm, bool wantEnergy, 
                 double& timeInPs, double& energyInKcal, double& pertenergyInKcal,
                 MyAtomInfo atoms[])
{
    int infoMask = 0;
