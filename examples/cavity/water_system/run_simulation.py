...
def run_simulation(lambda_coupling=0.01, cavity_freq_cm=1600.0, num_molecules=1000, 
    ...
    cavity_freq_cm : float
        Cavity frequency in cm⁻¹ (default: 1600 for HOH bend, 3663 for OH stretch)
    num_molecules : int
        Number of water molecules
    box_size_nm : float
        Box size in nm
    ...
    # Determine mode name
    mode_name = "HOH bending" if cavity_freq_cm < 2000 else "OH stretch"
    
    print(f"\nSimulation Parameters:")
    print(f"  Water molecules: {num_molecules}")
    print(f"  Box size: {box_size_nm:.2f} nm")
    print(f"  Temperature: {temperature_K} K")
    print(f"  Cavity frequency: {cavity_freq_cm:.0f} cm⁻¹ ({mode_name})")
    ...
    for step in range(total_steps):
        ...
        if (step + 1) % 1000 == 0 or (step + 1) == total_steps:
            ...
            if not disable_dipole_output and phase == "PROD" and len(dipole_times) > 0:
...
def main():
    ...
    parser.add_argument('--cavity-freq', type=float, default=1600.0, dest='cavity_freq',
                       help='Cavity frequency in cm⁻¹ (default: 1600 for HOH bend, use 3663 for OH stretch)')
