...
.. toctree::
    ...
    generated/OpenMM::State

State variables are not accessible directly via a ``Context``; in order to make
explicit the precise time that a variable reflects, a ``State`` object must be
constructed before data can be read from a simulation. They are created by
calling a method on a ``Context`` and only store the information needed at a
particular time.

.. toctree::
    :maxdepth: 2

    generated/OpenMM::Platform
