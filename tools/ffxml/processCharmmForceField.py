...
def createPatchedResidue(residue, patch):
    ...
    for atom in residue.atoms:
        if atom.name not in atomNames and atom.name not in patch.deletions:
            r.addAtom(atom)
    ...
    for atom in residue.atoms:
        if atom.polarizable:
            ...
            if atom.anisotropic:
                ...
                aniso = None
            drudes[(drude.type, atom.type)] = (atom.alpha.value_in_unit(nanometers**2/kilojoules_per_mole), atom.thole, atom.drudeCharge, aniso)
...
for residue in residues:
    ...
    for lp in residue.lonepairs:
        atoms = [residue.atomMap[lp[0]], residue.atomMap[lp[1]], residue.atomMap[lp[2]], residue.atomMap[lp[3]]]
        if lp[4] == 'relative':
        ...
        print('   <VirtualSite type="localCoords" index="%d" atom1="%d" atom2="%d" atom3="%d" wo1="1" wo2="0" wo3="0" wx1="%g" wx2="%g" wx3="%g" wy1="0" wy2="-1" wy3="1" p1="%g" p2="%g" p3="%g"/>' % tuple(atoms+xweights+p))
    print('  </Residue>')
print(' </Residues>')
