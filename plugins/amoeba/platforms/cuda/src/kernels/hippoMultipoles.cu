 ...
 */
extern "C" __global__ void computePotentialAtPoints(const real4* __restrict__ posq, const real* __restrict__ labDipole,
        const real* __restrict__ labQXX, const real* __restrict__ labQXY, const real* __restrict__ labQXZ,
        const real* __restrict__ labQYY, const real* __restrict__ labQYZ, const real* __restrict__ inducedDipole, const real4* __restrict__ points,
    ...
    extern __shared__ real4 sharedPosq[];
    real3* sharedDipole = (real3*) &sharedPosq[blockDim.x];
    real3* sharedInducedDipole = (real3*) &sharedDipole[blockDim.x];
    real* sharedQuadrupole = (real*) &sharedInducedDipole[blockDim.x];
    for (int basePoint = blockIdx.x*blockDim.x; basePoint < numPoints; basePoint += gridDim.x*blockDim.x) {
        ...
        for (int baseAtom = 0; baseAtom < NUM_ATOMS; baseAtom += blockDim.x) {
            ...
            if (atom < NUM_ATOMS) {
                sharedPosq[threadIdx.x] = posq[atom];
                sharedDipole[threadIdx.x] = make_real3(labDipole[3*atom], labDipole[3*atom+1], labDipole[3*atom+2]);
                sharedInducedDipole[threadIdx.x] = make_real3(inducedDipole[3*atom], inducedDipole[3*atom+1], inducedDipole[3*atom+2]);
                sharedQuadrupole[5*threadIdx.x] = labQXX[atom];
                sharedQuadrupole[5*threadIdx.x+1] = labQXY[atom];
                sharedQuadrupole[5*threadIdx.x+2] = labQXZ[atom];
                sharedQuadrupole[5*threadIdx.x+3] = labQYY[atom];
                sharedQuadrupole[5*threadIdx.x+4] = labQYZ[atom];
            }
            __syncthreads();
            ...
            if (point < numPoints) {
                ...
                for (int i = 0; i < end; i++) {
                    real3 delta = trimTo3(sharedPosq[i]-pointPos);
#ifdef USE_PERIODIC
                    APPLY_PERIODIC_TO_DELTA(delta)
#endif
                    real r2 = dot(delta, delta);
                    real rInv = RSQRT(r2);
                    p += sharedPosq[i].w*rInv;
                    real rr2 = rInv*rInv;
                    real rr3 = rInv*rr2;
                    real scd = dot(sharedDipole[i], delta);
                    real scu = dot(sharedInducedDipole[i], delta);
                    p -= (scd+scu)*rr3;
                    real rr5 = 3*rr3*rr2;
                    real scq = delta.x*dot(delta, make_real3(sharedQuadrupole[5*i+0], sharedQuadrupole[5*i+1], sharedQuadrupole[5*i+2])) +
                            delta.y*dot(delta, make_real3(sharedQuadrupole[5*i+1], sharedQuadrupole[5*i+3], sharedQuadrupole[5*i+4])) +
                            delta.z*dot(delta, make_real3(sharedQuadrupole[5*i+2], sharedQuadrupole[5*i+4], -sharedQuadrupole[5*i]-sharedQuadrupole[5*i+3]));
                    p += scq*rr5;
                }
            }
            __syncthreads();
