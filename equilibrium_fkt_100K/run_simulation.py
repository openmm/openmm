...
def _wrap_positions_centered(pos_nm, box_nm):
    """
    Wrap positions into [-L/2, L/2) per dimension (HOOMD convention for F(k,t) parity).
    """
    pos = np.asarray(pos_nm, dtype=np.float64).copy()
    box = np.asarray(box_nm, dtype=np.float64)
    if box.ndim == 1:
        box = box[:3]
    for d in range(3):
        ...
        if Ld <= 0:
            raise ValueError(f"Box length L[{d}] = {Ld} must be positive")
        # Shift so that [0, L) becomes [-L/2, L/2), then use round for nearest image
        pos[:, d] = pos[:, d] - np.round(pos[:, d] / Ld) * Ld
    return pos
...
def _load_gsd_config(path, frame_index):
    ...
    and velocities (Bohr per a.u. time, particle index order) if present.
    ...
    if hasattr(snap.particles, "velocity") and snap.particles.velocity is not None:
        out["velocities_bohr_per_autime"] = np.array(snap.particles.velocity, dtype=np.float64)
    return out
...
def _build_system_from_gsd(gsd_path, frame_index, gsd_in_nm, no_cavity, ff_dir,
    ...
    # Build velocities in same order as positions (Bohr/a.u. time -> nm/ps)
    vel_raw = cfg.get("velocities_bohr_per_autime")
    vel_scale = (scale / AU_TIME_TO_PS) if (vel_raw is not None and not gsd_in_nm) else (1.0 if vel_raw is not None else None)
    ...
    for b in range(n_mol):
        ...
        if vel_raw is not None:
            velocities_list.append(openmm.Vec3(float(vel_raw[i, 0] * vel_scale), float(vel_raw[i, 1] * vel_scale), float(vel_raw[i, 2] * vel_scale)) * (unit.nanometer / unit.picosecond))
            velocities_list.append(openmm.Vec3(float(vel_raw[j, 0] * vel_scale), float(vel_raw[j, 1] * vel_scale), float(vel_raw[j, 2] * vel_scale)) * (unit.nanometer / unit.picosecond))
    cavity_index = None
    if has_cavity:
    ...
    return system, positions_list, velocities_list, topology, cavity_index, n_mol, float(box_nm[0])


def _load_positions_and_box_from_gsd(path, frame_index, gsd_in_nm=False, bohr_to_nm=BOHR_TO_NM):
    """
    Load positions, velocities (if present), and box from a GSD file (cav-hoomd or HOOMD format).

    Units:
      - If gsd_in_nm is False (default): assumes GSD is in atomic units (Bohr);
        multiplies positions and box by bohr_to_nm to get nm.
        Velocities are in Bohr per atomic time unit; converted to nm/ps.
      - If gsd_in_nm is True: assumes GSD is already in nm (no conversion).
        Velocities still assumed in Bohr/a.u. time unless GSD is nm/ps (then no conversion).
    ...
    Returns (positions_list, velocities_list, box_nm, box_raw) where positions_list is list of
    openmm.Vec3 * unit.nanometer, velocities_list is list of openmm.Vec3 * (unit.nanometer/unit.picosecond)
    or None if GSD has no velocities, box_nm is (Lx, Ly, Lz) in nm.
    """
    if not HAS_GSD:
    ...
    # Load velocities if present (GSD from cav-hoomd: Bohr per atomic time unit)
    velocities_list = None
    if hasattr(frame.particles, "velocity") and frame.particles.velocity is not None:
        vel_raw = np.array(frame.particles.velocity, dtype=np.float64)
        if vel_raw.size > 0:
            # velocity in nm/ps = velocity_bohr * (BOHR_TO_NM / AU_TIME_TO_PS)
            if gsd_in_nm:
                vel_nm_ps = vel_raw  # assume already nm/ps
            else:
                vel_nm_ps = vel_raw * (bohr_to_nm / AU_TIME_TO_PS)
            velocities_list = [
                openmm.Vec3(float(vx), float(vy), float(vz)) * (unit.nanometer / unit.picosecond)
                for vx, vy, vz in vel_nm_ps
            ]

    return positions_list, velocities_list, box_nm, box_raw
...
def run_test(num_molecules=250, lambda_coupling=0.001, temperature_K=100.0,
    ...
    """
    if equil_only and not save_checkpoint:
        raise ValueError("equil_only requires save_checkpoint path")
    if prod_from_checkpoint and not checkpoint:
        raise ValueError("prod_from_checkpoint requires checkpoint path")
    if (equil_only or save_checkpoint) and (prod_from_checkpoint or checkpoint):
        raise ValueError("Cannot combine equil_only/save_checkpoint with prod_from_checkpoint/checkpoint")
    ...
    # Dipole output parameters (sampling less often greatly speeds up GPU runs)
    if dipole_output_interval_ps is None:
        dipole_output_interval_ps = dt
    dipole_output_interval_steps = max(1, int(round(dipole_output_interval_ps / dt)))
    ...
    if not disable_dipole_output:
        print(f"  Dipole output: every {dipole_output_interval_steps} steps ({dipole_output_interval_ps*1000:.2f} fs) from t=0 for full run")
    else:
    ...
    if prod_from_checkpoint:
        ...
        chk_path = Path(checkpoint)
        if not chk_path.exists():
            raise FileNotFoundError(f"Checkpoint not found: {chk_path}")
        with open(chk_path, "rb") as f:
            chk_data = f.read()
        context.loadCheckpoint(chk_data)
        print(f"\n--- Loaded checkpoint from {chk_path} ---")
        print("  Production starts from equilibrated (pos, vel); no re-thermalization")
    else:
    ...
    if not prod_from_checkpoint:
        if build_from_gsd and velocities_from_gsd is not None and not restart_velocities:
            context.setVelocities(velocities_from_gsd)
            print("\n--- Velocities loaded from GSD ---")
            state = context.getState(getVelocities=True)
            vels_check = state.getVelocities(asNumpy=True).value_in_unit(unit.nanometer / unit.picosecond)
            vel_rms = np.sqrt(np.mean(vels_check**2))
            print(f"  Velocity RMS: {vel_rms:.6f} nm/ps")
        else:
            context.setVelocitiesToTemperature(temperature_K * unit.kelvin)
            # Remove COM velocity (HOOMD parity: thermalize_particle_momenta zeros COM).
            # Without this, net COM drift adds phase exp(i k·v_COM·t) to F(k,t),
            # causing spurious oscillation and fast decorrelation.
            state = context.getState(getVelocities=True)
            vels_np = state.getVelocities(asNumpy=True).value_in_unit(unit.nanometer / unit.picosecond)
            masses_arr = np.array([system.getParticleMass(i).value_in_unit(unit.dalton) for i in range(num_molecular_particles)])
            total_mass = np.sum(masses_arr)
            v_com = np.sum(masses_arr[:, None] * vels_np[:num_molecular_particles], axis=0) / total_mass
            vels_np[:num_molecular_particles] -= v_com[None, :]
            vel_list = [openmm.Vec3(float(v[0]), float(v[1]), float(v[2])) * (unit.nanometer / unit.picosecond) for v in vels_np]
    ...
    if equil_only:
        ...
        _equil_steps_for_fkt = equilibration_steps  # F(k,t) not used in equil_only
    elif prod_from_checkpoint:
        total_steps = production_steps
        _equil_steps_for_fkt = 0  # F(k,t) runs from step 0 (we're in prod)
    else:
        total_steps = equilibration_steps + production_steps
        _equil_steps_for_fkt = equilibration_steps
    
    print("\n--- Running Simulation ---")
    if equil_only:
        print(f"  Mode: Equilibration only ({equilibration_time_ps} ps, {total_steps} steps)")
    elif prod_from_checkpoint:
        print(f"  Mode: Production from checkpoint ({production_time_ps} ps, {total_steps} steps)")
    else:
        print(f"  Equilibration: {equilibration_time_ps} ps ({equilibration_steps} steps)")
        print(f"  Production:    {production_time_ps} ps ({production_steps} steps)")
    ...
    for i in range(num_reports):
        ...
        for step in range(steps_this_block):
            ...
            if pdb_handle and topology_molecular is not None and (step_counter % pdb_interval_steps) == 0:
                ...
                pdb_model_index += 1
            # F(k,t): only track during production (after equilibration); use production-relative time
            # so t=0 corresponds to the start of production (cav-hoomd parity).
            if enable_fkt and fkt_tracker is not None and step_counter >= _equil_steps_for_fkt:
                ...
                if not fkt_initialized:
                    ...
                    def _to_nm_fkt(x):
                        return x.value_in_unit(unit.nanometer) if hasattr(x, 'value_in_unit') else float(x)
                    box_nm0 = np.array([_to_nm_fkt(a0[0]), _to_nm_fkt(b0[1]), _to_nm_fkt(c0[2])])
        ...
        # Save NPZ file periodically (not every report, to avoid I/O bottleneck)
        if (not disable_dipole_output and len(dipole_times) > 0 and
                npz_save_interval_reports > 0 and (i + 1) % npz_save_interval_reports == 0):
    ...
    # Equil-only: save checkpoint and exit (two-phase protocol; cav-hoomd parity)
    if equil_only:
        chk_path = Path(save_checkpoint)
        chk_path.parent.mkdir(parents=True, exist_ok=True)
        chk_data = context.createCheckpoint()
        safesave.save(chk_data, str(chk_path))
        print(f"\n--- Equilibration complete; checkpoint saved to {chk_path} ---")
...
Examples:
  ...
  python run_simulation.py --nve --dimers 50 --equil 0 --prod 100
    ...
    parser.add_argument('--dipole-interval-ps', type=float, default=None, dest='dipole_interval_ps',
                        help='Dipole sample interval in ps (default: dt = every step). Use 0.01 or 0.1 for faster runs; IR spectrum still fine with ~10–100 fs resolution.')
    parser.add_argument('--npz-save-interval-reports', type=int, default=100, dest='npz_save_interval_reports',
                        help='Save dipole .npz every N console reports (default: 100). Set 0 to save only at end.')
    ...
    parser.add_argument('--fkt-kmag', type=float, default=18.9, dest='fkt_kmag',
                        help='F(k,t) k-magnitude in nm⁻¹ (default: 18.9 = 1 Bohr⁻¹ for run_equilibrium_fkt parity; cav-hoomd uses 1.0 in Bohr)')
    ...
    parser.add_argument('--initial-gsd', type=str, default='init-0.gsd', dest='initial_gsd',
                        help='Load initial positions and box from this GSD file (Bohr->nm); skip minimization (default: init-0.gsd)')
    ...
    parser.add_argument('--restart-velocities', action='store_true', dest='restart_velocities',
    ...
    parser.add_argument('--equil-only', action='store_true', dest='equil_only',
                        help='Run equilibration only; requires --save-checkpoint')
    parser.add_argument('--save-checkpoint', type=str, default=None, dest='save_checkpoint',
                        help='Path to save checkpoint after equilibration (use with --equil-only)')
    parser.add_argument('--prod-from-checkpoint', action='store_true', dest='prod_from_checkpoint',
                        help='Load checkpoint and run production only; requires --checkpoint')
    parser.add_argument('--checkpoint', type=str, default=None, dest='checkpoint',
                        help='Path to checkpoint file (use with --prod-from-checkpoint)')
    ...
    try:
        success = run_test(
            ...
            dipole_output_interval_ps=args.dipole_interval_ps,
            box_size_nm=box_size_nm,
            ...
            npz_save_interval_reports=args.npz_save_interval_reports,
            ...
            prod_from_checkpoint=args.prod_from_checkpoint,
