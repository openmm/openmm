...
extern "C" OPENMM_EXPORT void registerKernelFactories() {
    ...
    for (int i = 0; i < Platform::getNumPlatforms(); i++) {
        ...
        if (dynamic_cast<ReferencePlatform*>(&platform) != NULL) {
             ...
             platform.registerKernelFactory(CalcAmoebaCTForceKernel::Name(), factory);
...
KernelImpl* AmoebaReferenceKernelFactory::createKernelImpl(std::string name, const Platform& platform, ContextImpl& context) const {
    ...
    if (name == CalcAmoebaCTForceKernel::Name())
        return new ReferenceCalcAmoebaCTForceKernel(name, platform, context.getSystem());
