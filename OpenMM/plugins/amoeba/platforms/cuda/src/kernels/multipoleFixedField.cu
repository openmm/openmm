...
inline __device__ void loadAtomData(AtomData& data, int atom, const real4* __restrict__ posq, const real* __restrict__ labFrameDipole, const real* __restrict__ labFrameQuadrupole, const float2* __restrict__ dampingAndThole, const real* __restrict__ dirDamping) {
    data.posq = posq[atom];
...
//Chengwen_Comments:
//This functions as the direct induction in induce.f code
//Aug.15th

//Ewald method;
//DONE with new dampings
#ifdef USE_EWALD
__device__ void computeOneInteraction(AtomData& atom1, AtomData& atom2, real3 deltaR, float dScale, float pScale, real3* fields) {
    ...
    if (r2 <= CUTOFF_SQUARED) {
        ...
        if (damp != 0) {
            real ratio = SQRT(r/damp);
            ratio = ratio*ratio*ratio;
            real pgamma = (atom1.dirdamp < atom2.dirdamp ? atom1.dirdamp : atom2.dirdamp);
            damp = pgamma*ratio;
            if (damp > -50) {
                real expdamp = EXP(-damp);
                scale3 = 1 - expdamp;
                scale5 = 1 - expdamp*(1+0.5f*damp);
                scale7 = 1 - expdamp*(1+0.65f*damp+0.15f*damp*damp);
            }
        }
        ...
        real qkr = dot(qk, deltaR);//same as qrrk, vector though
...
#else
//Non-Ewald
//CPU code: direct induction via double loop
//Done with new damping factors
__device__ void computeOneInteraction(AtomData& atom1, AtomData& atom2, real3 deltaR, float dScale, float pScale, real3* fields) {
    ...
    if (damp != 0) {
        ...
        real ratio = SQRT(r/damp);
        float pGamma = atom2.dirdamp > atom1.dirdamp ? atom1.dirdamp : atom2.dirdamp; 
    ...
    rr3 *= 1 - dampExp;
    rr5 *= 1 - (1+0.5f*damp)*dampExp;
    rr7 *= 1 - (1+0.65f*damp+(0.15f*damp*damp))*dampExp;
...
extern "C" __global__ void computeFixedField(
...
#endif
        const real* __restrict__ labFrameDipole, const real* __restrict__ labFrameQuadrupole, const float2* __restrict__ dampingAndThole, const real* __restrict__ dirDamping) {
    ...
    for (int pos = firstExclusionTile; pos < lastExclusionTile; pos++) {
        ...
        loadAtomData(data, atom1, posq, labFrameDipole, labFrameQuadrupole, dampingAndThole, dirDamping);
#ifdef USE_GK
        ...
        if (x == y) {
            ...
            localData[localAtomIndex].dirdamp = data.dirdamp;
#ifdef USE_GK
            ...
            loadAtomData(localData[localAtomIndex], j, posq, labFrameDipole, labFrameQuadrupole, dampingAndThole, dirDamping);
