...
public:
    ...
    bool areParticlesIdentical(int particle1, int particle2) {
        double charge1, charge2, penalpha1, penalpha2, thole1, thole2, dirdamp1, dirdamp2, damping1, damping2, polarity1, polarity2;
        int axis1, axis2, multipole11, multipole12, multipole21, multipole22, multipole31, multipole32, atomic1, atomic2;
        vector<double> dipole1, dipole2, quadrupole1, quadrupole2;
        force.getMultipoleParameters(particle1, charge1, dipole1, quadrupole1, axis1, multipole11, multipole21, multipole31, penalpha1, atomic1, thole1, dirdamp1, damping1, polarity1);
        force.getMultipoleParameters(particle2, charge2, dipole2, quadrupole2, axis2, multipole12, multipole22, multipole32, penalpha2, atomic2, thole2, dirdamp2, damping2, polarity2);
        if (charge1 != charge2 || penalpha1 != penalpha2 || atomic1 !=atomic2 || thole1 != thole2 || dirdamp1 != dirdamp2 || damping1 != damping2 || polarity1 != polarity2 || axis1 != axis2) {
            return false;
        }
...
CudaCalcAmoebaMultipoleForceKernel::CudaCalcAmoebaMultipoleForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
        ...
        fracDipoles(NULL), fracQuadrupoles(NULL), field(NULL), fieldPolar(NULL), inducedField(NULL), inducedFieldPolar(NULL), torque(NULL), penAlpha(NULL), atomicNum(NULL), dampingAndThole(NULL), dirDamping(NULL), inducedDipole(NULL),
...
void CudaCalcAmoebaMultipoleForceKernel::initialize(const System& system, const AmoebaMultipoleForce& force) {
    ...
    for (int i = 0; i < numMultipoles; i++) {
        ...
        int axisType, atomX, atomY, atomZ, atomic;
        vector<double> dipole, quadrupole;
        force.getMultipoleParameters(i, charge, dipole, quadrupole, axisType, atomZ, atomX, atomY, penalpha, atomic, thole, dirdamp, damping, polarity);
        //printf("\n penalpha charge %i %i %f %f \n", i, numMultipoles, penalpha, charge); //PASSED
        ...
        atomicNumVec.push_back((int) atomic);
    ...
    atomicNum = CudaArray::create<int>(cu, paddedNumAtoms, "atomicNum");
    ...
    if (pmeGrid == NULL) {
        ...
        if (gkKernel == NULL) {
            void* computeFixedFieldArgs[] = {&field->getDevicePointer(), &fieldPolar->getDevicePointer(), &cu.getPosq().getDevicePointer(),
                ...
                &labFrameDipoles->getDevicePointer(), &labFrameQuadrupoles->getDevicePointer(), &penAlpha->getDevicePointer(), &atomicNum->getDevicePointer(), &dampingAndThole->getDevicePointer(), &dirDamping->getDevicePointer()};
        ...
        void* electrostaticsArgs[] = {&cu.getForce().getDevicePointer(), &torque->getDevicePointer(), &cu.getEnergyBuffer().getDevicePointer(),
            ...
            &inducedDipole->getDevicePointer(), &inducedDipolePolar->getDevicePointer(), &penAlpha->getDevicePointer(), &atomicNum-> getDevicePointer(), &dampingAndThole->getDevicePointer(), &dirDamping->getDevicePointer()};
        ...
        if (gkKernel != NULL)
            gkKernel->finishComputation(*torque, *labFrameDipoles, *labFrameQuadrupoles, *inducedDipole, *inducedDipolePolar, *penAlpha, *atomicNum, *dampingAndThole, *dirDamping, *covalentFlags, *polarizationGroupFlags);
    }
    else {
...
void CudaCalcAmoebaGeneralizedKirkwoodForceKernel::initialize(const System& system, const AmoebaGeneralizedKirkwoodForce& force) {
    ...
    for (int i = 0; i < force.getNumParticles(); i++) {
        ...
        double charge2, penalpha, thole, dirdamp, damping, polarity;
        int axisType, atomX, atomY, atomZ, atomic;
        vector<double> dipole, quadrupole;
        multipoles->getMultipoleParameters(i, charge2, dipole, quadrupole, axisType, atomZ, atomX, atomY, penalpha, atomic, thole, dirdamp, damping, polarity);
        if (charge != charge2)
...
void CudaCalcAmoebaGeneralizedKirkwoodForceKernel::finishComputation(CudaArray& torque, CudaArray& labFrameDipoles, CudaArray& labFrameQuadrupoles,
            CudaArray& inducedDipole, CudaArray& inducedDipolePolar, CudaArray& penAlpha, CudaArray& atomicNum, CudaArray& dampingAndThole, CudaArray& dirDamping, CudaArray& covalentFlags, CudaArray& polarizationGroupFlags) {
    CudaNonbondedUtilities& nb = cu.getNonbondedUtilities();
    int startTileIndex = nb.getStartTileIndex();
    int numTileIndices = nb.getNumTiles();
    int numForceThreadBlocks = nb.getNumForceThreadBlocks();
