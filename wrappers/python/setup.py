...
def getLibraries():
    libraries = ['OpenMM', 'OpenMMAmoeba', 'OpenMMRPMD', 'OpenMMDrude']

    if 'OPENMM_USE_DEBUG_LIBS' in os.environ:
        if platform.system() == "Windows":
            raise Exception("use of OpenMM debug libs not supported on Win OS")
        else:
            print("WARNING: using debug libs")
            libraries = ['%s_d' % l for l in libraries]
            print(libraries)
    return libraries
...
def writeVersionPy(filename="simtk/openmm/version.py",
...
"""
    ...
    version = full_version = '%s.%s.%s' % (major_version_num,
                                           minor_version_num, build_info)
    if not IS_RELEASED:
