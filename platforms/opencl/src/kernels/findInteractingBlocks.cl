            ...
            for (int i = 0; i < TILE_SIZE; i++) {
                ...
                if (i < TILE_SIZE) {
                    ...
                    if (neighborsInBuffer > BUFFER_SIZE-TILE_SIZE) {
                        ...
                        if (indexInWarp == 0)
                            *tileStartIndex = ATOMIC_ADD(interactionCount, (TileIndex)tilesToStore);
                        SYNC_WARPS;
...
void storeInteractionData(int x, __local int* buffer, __local int* sum, __local int2* temp, __local int* atoms, __local int* numAtoms,
    ...
    for (int base = 0; base < numValid; base += BUFFER_SIZE/WARP_SIZE) {
        ...
        if (tilesToStore > 0) {
            if (get_local_id(0) == 0)
                *baseIndex = ATOMIC_ADD(interactionCount, (TileIndex)tilesToStore);
            barrier(CLK_LOCAL_MEM_FENCE);
            if (get_local_id(0) == 0)
    ...
    if (numValid == 0 && *numAtoms > 0 && finish) {
        ...
        if (get_local_id(0) == 0)
            *baseIndex = ATOMIC_ADD(interactionCount, (TileIndex)1);
        barrier(CLK_LOCAL_MEM_FENCE);
