...
#if USE_ESP
real tempForce = 0.0f;
real tableX = r*ESP_INV_DX;
int tableIndex = max(min((int) floor(tableX), ESP_NUM_TABLE_POINTS), 0);
real coeff2 = tableX-tableIndex;
real coeff1 = 1.0f-coeff2;
real erfcVal = coeff1*ESP_ENERGY_TABLE[tableIndex] + coeff2*ESP_ENERGY_TABLE[tableIndex+1];
real scaleVal = coeff1*ESP_SCALE_TABLE[tableIndex] + coeff2*ESP_SCALE_TABLE[tableIndex+1];
real erfLike = 1.0f-erfcVal;
if (erfLike > 1e-6f) {
    const real prefactor = exclusionParams.x*invR;
    tempForce = -prefactor*(1.0f-scaleVal);
    energy -= prefactor*erfLike;
}
#else
